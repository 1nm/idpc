...
class IDPhotoCreator:
    def __init__(self, photo_size, canvas_size, dpi):
        ...
        bw, bh = self._mm2px((10, 10))  # 5mm blank for each edge
    ...
    def create(self, photo, output):
        ...
        if w >= self.pw and h >= self.ph:
            ...
            print('Image size must be no smaller than {}x{} while size of input is {}x{}'.format(self.pw, self.ph, w, h))
...
def main():
    ...
    parser.add_argument('--photo-size', nargs='?', default='35x45', help='widthxheight in mm, default value is 35x45')
    parser.add_argument('--canvas-size', nargs='?', default='89x127', help='widthxheight in mm, default value is 89x127 (Japanese L ban)')
    ...
    canvas_size = map(int, args.canvas_size.split('x'))
